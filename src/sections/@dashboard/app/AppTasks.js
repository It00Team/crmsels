...
export default function AppTasks({ title, subheader, list, ...other }) {
  ...
  const updateTaskCompletion = async (taskId) => {
    try {
      ...
      const res = await MyAPI.put(`/crm/task/update-task/${taskId}`, data, token);
      console.log(res)
            ...
            <>
              {list.map((task) => (
                <TaskItem
                  key={task.id}
                  task={task}
                  checked={completedTasks.includes(task.id) || task.status == 'Completed'} 
                  onChange={() => {
                    onSelected(task.id);
...
function TaskItem({ task, checked, onChange }) {
  ...
  const handleEdit = () => {
    handleCloseMenu();
    navigate(`/dashboard/to-do-task/${task.id}`)
